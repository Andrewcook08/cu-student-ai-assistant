...
- [Epic 12: Launch Prep](#epic-12-launch-prep)
...
- **Assignee**: Person B
- **Labels**: `critical-path`
- **Status**: ✅ Done (CUAI-32 spike; harness retired post-migration, see [ADR-50](decisions.md#adr-50-gpu-vm-test-harness--abandoned))
...
- **Assignee**: Person B
- **Status**: ✅ Done — GET only. PUT /api/students/me/completed-courses split into sibling story API-005b.
...
- **Assignee**: Person B
- **Status**: ✅ Done (CUAI-78, PR #66)
- **Description**: `PUT /api/students/me/completed-courses` lets an authenticated student update their self-reported completed course list with optional grades. Split out of API-005 so the GET work could close cleanly. Tracked as CUAI-78.
- **Acceptance criteria**:
...
- **Assignee**: Person C
- **Status**: ✅ Done (spike — research folded into CHAT-008)
...
- **Acceptance criteria**:
  ...
  - [ ] Timeboxed — findings shared regardless of completion
...
- **Assignee**: Person C
- **Status**: ✅ Done (CUAI-35, PR #70; Anthropic migration in CUAI-87, PR #120)
...
- **Assignee**: Person C
- **Status**: ✅ Done (CUAI-36, PR #72; inference-queue portion later abandoned per [ADR-49](decisions.md#adr-49-redis-retained-for-session-storage-inference-queue-abandoned) and removed in post-migration cleanup commit `6739f3a`)
...
- **Blocked by**: FE-001
- **Assignee**: Person B
- **Status**: ✅ Done (bundled in CUAI-47, PR #58)
...
### SEC-005..009: API & Infrastructure Hardening (retrofit)
...
- **Status**: ✅ Done (CUAI-79, PR #85; scope narrowed in PR #138 per [ADR-43](decisions.md#adr-43-public-read-authenticated-write-for-catalog-routes))
...
- **Blocked by**: DEPLOY-001
- **Assignee**: Person A
- **Status**: ✅ Done (bundled in DEPLOY-004 CUAI-67, PR #123)
...
- **Assignee**: Person B
- **Status**: ✅ Done (CUAI-72, PR #127; WIF auth in CUAI-89, PR #126)
...
- **Assignee**: Everyone
- **Status**: 📋 Planned
- **Description**: Write a demo script with 3-4 compelling scenarios. Practice the demo. Prepare backup plan (recorded video) in case of live issues.
- **Acceptance criteria**:
...
- **Acceptance criteria**:
  ...
  - [ ] Presentation is concise and covers all key areas
...
|-------|--------|----------|
| INFRA-001 | 3 | Person C (Andrew) |
| INFRA-002 | 5 | Person A (Scott) |
| INFRA-003 | 3 | Person A (Scott) |
| CICD-001 | 2 | Person B |
| FE-001 | 3 | Person B |
| FE-002 | 4 | Person B |
| FE-003 | 4 | Person B |
...
**Per-person**: A (Scott)=8, B (Rohan)=29, C (Andrew)=25. Andrew bootstraps the repo skeleton first, then pivots to data ingestion (can start parsing logic in pure Python while Docker builds). Scott starts on shared package once the skeleton is merged. CHAT-000 is a timeboxed spike that overlaps with the start of Sprint 2.
...
**Per-person**: A=14, B=31, C=35. Person A's Sprint 2 load is the five SEC-005..009 retrofit tickets (added per ADR-33). They have no inter-dependencies except SEC-008 → SEC-006, and no external blockers — Scott has no in-progress tickets at the time of assignment. CHAT-011 may spill into the next sprint.
...
**Per-person**: A=11, B=17, C=15. Person A (Scott) owns conversation memory. Person C (Andrew) focuses on security hardening + prompt tuning (DEMO-001 moved up since its blocker CHAT-008 is now done). SEC-010 added for security headers middleware.
...
| Story | Points | Assignee |
|-------|--------|----------|
| DEPLOY-001 | 5 | Person A |
| DEPLOY-002 | 3 | Person A |
| ~~DEPLOY-003~~ | ~~5~~ | ~~Person A~~ |
| DEPLOY-004 | 3 | Person A |
| DEPLOY-008 | 3 | Person A |
| DEPLOY-005 | 1 | Person A |
| DEPLOY-006 | 2 | Person A |
| DEPLOY-007 | 2 | Person A |
| CICD-002 | 3 | Person B |
| DEMO-002 | 3 | Everyone |
| DEMO-003 | 3 | Everyone |
| **Total** | **30** | |

**Per-person**: A=21, B=3, C=0, Everyone=6. Person A heavy on Terraform. Person B lighter — can help with branding polish and bug fixes. DEMO-001 moved up to the prior sprint.
